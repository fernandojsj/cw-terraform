...
resource "aws_cloudwatch_metric_alarm" "rds_cpu_utilization" {
  for_each = { for idx, rds in local.RDS.t_instance_list : rds["id"] => rds if length(rds["id"]) > 0 }
  ...
  alarm_description   = "CRITICAL"
  alarm_actions       = [var.rds_alarm_action_arn]
  ok_actions          = [var.rds_alarm_action_arn]
}
...
resource "aws_cloudwatch_metric_alarm" "rds_std_cpu_utilization" {
  for_each = { for idx, rds in local.RDS.rds_list : rds["id"] => rds if length(rds["id"]) > 0 }
