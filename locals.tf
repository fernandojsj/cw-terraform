locals {
  ...
  map_of_rds = {
    aurora_list            = split(", ", local.rds_result["Aurora_list"])
    rds_list               = split(", ", local.rds_result["RDS"])
    aurora_serverless_list = split(", ", local.rds_result["Aurora_serverless_list"])
  }
  
  RDS = {
    rds_list               = length(local.rds_result["RDS"]) > 0 && local.rds_result["RDS"] != "" ? jsondecode(local.rds_result["RDS"]) : []
    aurora_list            = length(local.rds_result["Aurora_list"]) > 0 && local.rds_result["Aurora_list"] != "" ? split(", ", local.rds_result["Aurora_list"]) : []
    aurora_serverless_list = length(local.rds_result["Aurora_serverless_list"]) > 0 && local.rds_result["Aurora_serverless_list"] != "" ? split(", ", local.rds_result["Aurora_serverless_list"]) : []
  ...
  aurora_block_height = length(local.RDS.aurora_list) * 14
  ...
  aurora_header_y = local.rds_header_y + length(local.RDS.rds_list) * 14 + length(local.RDS.t_instance_list) * 14 + 4
